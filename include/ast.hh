...
struct Node {
    enum NodeType {
        BIN_OP, INT_LIT, STMTS, ASSN, DBG, IDENT, UPDT,TERN_OP, COND
    } type;
...
/**
    Node for variable UPDATION
*/
struct NodeUpdt : public Node {
    std::string identifier;
    Node *expression;

    NodeUpdt(std::string id, Node *expr);
    std::string to_string();
    llvm::Value *llvm_codegen(LLVMCompiler *compiler);
};
...
struct NodeTernOp : public Node {
    ...
    NodeTernOp(Node *evalptr, Node *tptr, Node *fptr);
    std::string to_string();
    llvm::Value *llvm_codegen(LLVMCompiler *compiler);
};

struct NodeConditional : public Node {
    
    Node *eval, *t, *f;

    NodeConditional(Node *evalptr, Node *tptr, Node *fptr);
    std::string to_string();
    llvm::Value *llvm_codegen(LLVMCompiler *compiler);
};
